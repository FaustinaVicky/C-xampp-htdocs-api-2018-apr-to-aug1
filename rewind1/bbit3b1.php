 ...
 */
  // function rewindtext($text){

  //   // $clean = array_slice()
  //   // $clean = preg_replace(".'.", ". ",$text);

  //   // $cleaner = preg_replace("", ".", $clean);
 ...
 $lines = explode("|","'Joy is the serious business of heaven.'.LEWIS,CLIVE STAPLES.1964-01-01.https://bit.ly/2HwPJd6
...
<div class="container">
<div class="well">
<h1>Quotes from the past</h1>
<ol class="list-group">
  <?php foreach($lines as $line){

    // quote part
   $endofquote = strrpos($line,".'.");
   $lastwords = substr($line, $endofquote);
   $quote = str_replace("'","",substr($line, 0, $endofquote));

// first name part
   $explodedend = explode(".h",$lastwords);
...
// last name
  ...
  $endoflname = strpos($lastparts,".");
  $lname= substr($lastparts, 0,$endoflname);
// date
  $dates = substr($lastparts,$endoflname);
  $endofdate = strrpos($dates,".");
  $date = date("F jS, Y", strtotime(str_replace( ".https://bit."," ",substr($dates, 1,$endofdate))));
